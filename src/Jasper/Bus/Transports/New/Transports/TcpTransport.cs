    ...
    public class TcpTransport : INewTransport
    {
        ...
        public void StartListening(NewBusSettings settings)
        {
            ...
            if (incoming.Any())
            {
                ...
                _replyUri = $"{Protocol}://{settings.MachineName}:{port}/{TransportConstants.Replies}"
                    .ToUri();
            }
        }

        private Listener buildDurableListener(Uri uri, NewBusSettings settings)
        {
        ...
        private Listener buildLightweightListener(Uri uri, NewBusSettings settings)
        {
            var agent = new SocketListeningAgent(uri.Port, settings.Cancellation);
